...
if [[ "${IMAGE_VERSION:-}" == "" ]]; then
    export IMAGE_VERSION=$UNIFIED_CI_IMAGE_VER
fi
...
export IMG="${IMAGE_NAME}:${IMAGE_VERSION}"
...
ENV_BLOCKLIST="${ENV_BLOCKLIST:-^_\|^PATH=\|^GOOS=\|^GOBIN=\|^GOPATH=\|^GOROOT=\|^SHELL=\|^EDITOR=\|^TMUX=\|^USER=\|^HOME=\|^PWD=\|^TERM=\|^rvm=\|^SSH=\|^TMPDIR=\|^CC=\|^CXX=\|^MAKEFILE_LIST=|^LC_}"
